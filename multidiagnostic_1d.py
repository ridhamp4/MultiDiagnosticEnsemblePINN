...
class EnsembleAllenCahnPINN(nn.Module):
    """Allen-Cahn PINN with your complete ensemble analysis framework"""
    
    def __init__(self, layers=[2, 64, 64, 64, 1], epsilon=0.001, alpha=5.0, domain_bounds=[(-1, 1)]):
        super(EnsembleAllenCahnPINN, self).__init__()
    ...
    def compute_allen_cahn_residual(self, x, u):
        """Allen-Cahn residual: u_t - εu_xx + α(u³ - u) = 0"""
    ...
    def compute_ensemble_adaptive_loss(self, data_dict, epoch):
        ...
        if epoch % 20 == 0:
            with torch.no_grad():
                ...
                # Use the ensemble analyzers as a separate loss term. Combine all
                # analyzer metrics with equal weight, except 'information' which is
                # ignored (weight 0). This produces an ensemble_term scalar that is
                # added to total_loss (multiplied by ensemble_loss_coeff below).
                ...
                # print(f"Epoch {epoch}: Ensemble term = {ensemble_term.item():.3f}")
                # print(f"  Analyzer weights: {ensemble_weight_vals}")
                
        # Note: we no longer use an "ensemble_boost" multiplicative term.
        # Ensemble guidance is recorded as an additive `ensemble_term` above
        # (and stored in self.history['ensemble_terms']). If needed, one
        # could reconstruct a boost from that term, but for now keep the
